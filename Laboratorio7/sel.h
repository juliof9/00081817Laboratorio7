...
Matrix createLocalK(int e,mesh &m){
    ...
    Matrix matrixA,matrixK,matrixD,matrixH,matrixEE;
    ...
    productRealMatrix((2*J*lambda)/(18*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixD);
    ...
    //E
    productRealMatrix(J/(6*D),productMatrixMatrix(BPrimat,productMatrixMatrix(Alphat,g_matrix_t,2,2,6),3,2,6),matrixEE);

    //H
    delta = m.getParameter(DELTHA);
    productRealMatrix((-1)*(delta*Ae)/(8*D*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
    ...
    ubicarSubMatriz(K,0,5,6,8,sumMatrix(matrixD,matrixH,6,3));
    ubicarSubMatriz(K,6,8,0,5,matrixEE);
